...
function composeUserMessage(reason, err, provider, model) {
    ...
    switch (reason) {
        case "auth": {
            ...
            const keyRequired = msg.includes("is required") || msg.includes("No API key configured");
...
function diagnose(err) {
    if (err instanceof ProviderError) {
        ...
        if (status === 429)
            return { reason: "rate_limit", backoffMs: 4000 };
        if (status === 503 || status === 529)
            return { reason: "overloaded", backoffMs: 3000 };
        if (status >= 500)
            return { reason: "overloaded", backoffMs: 2000 };
        ...
        if (msg.includes("context length"))
            return { reason: "context_overflow" };
...
function shouldFallback(reason) {
    return (reason === "rate_limit" ||
        reason === "overloaded" ||
