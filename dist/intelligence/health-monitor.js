...
function loadPersisted() {
    try {
        ...
        for (const [_provider, entry] of Object.entries(store.providers)) {
            ...
            s.totalRequests = entry.totalRequests ?? 0;
            s.failures = entry.failures ?? 0;
            s.lastFailure = entry.lastFailure ?? 0;
            s.lastSuccess = entry.lastSuccess ?? 0;
            s.avgLatencyMs = entry.avgLatencyMs ?? 0;
            s.blacklistedUntil = entry.blacklistedUntil ?? null;
            s.cooldownLevel = entry.cooldownLevel ?? 0;
...
export function isBlacklisted(provider) {
    ...
    if (!s)
        return false;
    if (s.blacklistedUntil === null)
        return false;
    if (Date.now() < s.blacklistedUntil)
        return true;
