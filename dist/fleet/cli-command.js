 ...
 */
export async function fleetCommand(argv, opts = {}) {
    const [sub, ...rest] = argv;
    switch (sub) {
        case undefined:
        case "--help":
...
/* ---------------------------- subcommands ------------------------- */
async function doLaunch(argv, opts) {
...
/* -------------------------- emit helpers -------------------------- */
function emitResult(phase, r, opts) {
    if (opts.json) {
...
function printHelp() {
    process.stdout.write(`dirgha fleet — parallel multi-agent work in isolated git worktrees
 ...
 */
export function registerFleetSlash(registry, opts = {}) {
