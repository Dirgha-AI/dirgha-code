...
import { EMBEDDING_DIM } from "./iface.js";
...
function tryRequire() {
    ...
    _resolved = true;
    try {
        _transformers = _require("@xenova/transformers");
    }
    catch {
        _transformers = null;
    }
...
export function isLocalEmbeddingAvailable() {
...
async function getPipeline() {
    const tx = tryRequire();
    if (!tx) {
        throw new Error("@xenova/transformers is not installed. Reinstall dirgha without --omit=optional, " +
    ...
    if (!_pipelinePromise) {
        ...
        _pipelinePromise = tx.pipeline("feature-extraction", MODEL_ID);
    }
...
export class LocalEmbedder {
    ...
    async embed(texts) {
        ...
        const flat = out.data;
        const dim = Array.isArray(out.dims) && out.dims.length >= 2
...
export function _resetLocalEmbedderForTests() {
    _transformers = null;
    _resolved = false;
