...
export function DenseToolMessage(props) {
    ...
    const glyph = props.status === 'error' ? TOOL_STATUS.ERROR
        : props.status === 'done' ? TOOL_STATUS.SUCCESS
            : SPINNER[frame];
    const glyphColour = props.status === 'error' ? palette.status.error
        : props.status === 'done' ? palette.status.success
            : palette.status.warning;
    const nameColour = props.status === 'done' ? palette.text.secondary
