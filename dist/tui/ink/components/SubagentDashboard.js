...
function labelFromInput(input) {
    if (input && typeof input === "object" && "prompt" in input) {
        ...
        if (typeof p === "string" && p.trim())
            return truncate(p.trim(), MAX_LABEL_LEN);
    }
    return "sub-agent";
}
...
/*  Component                                                          */
/* ------------------------------------------------------------------ */
export function SubagentDashboard(props) {
    ...
    return (_jsxs(Box, { flexDirection: "column", paddingX: 1, marginBottom: 0, children: [_jsxs(Text, { color: palette.text.secondary, children: ["agents (", entries.length, ")"] }), entries.map((s) => (_jsxs(Box, { flexDirection: "column", children: [_jsxs(Text, { children: ["  ", _jsxs(Text, { color: statusColor(s.status), children: [statusDot(s.status), " "] }), _jsx(Text, { color: palette.text.primary, children: s.label }), _jsxs(Text, { color: palette.text.secondary, children: [" ", formatDuration(s.durationMs)] })] }), s.status === "completed" && s.output && (_jsxs(Text, { color: palette.ui.comment, children: ["    ", outputPreview(s.output)] })), s.status === "error" && s.error && (_jsxs(Text, { color: palette.status.error, children: ["    ", outputPreview(s.error)] }))] }, s.id)))] }));
}
//# sourceMappingURL=SubagentDashboard.js.map