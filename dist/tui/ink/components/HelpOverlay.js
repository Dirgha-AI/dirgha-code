...
export function HelpOverlay(props) {
    ...
    for (const group of GROUP_ORDER) {
        ...
        lines.push(_jsx(Box, { marginTop: 1, children: _jsx(Text, { color: palette.accent, bold: true, children: GROUP_TITLES[group] ?? group }) }, `h-${group}`));
        for (const c of cmds) {
            lines.push(_jsxs(Box, { gap: 2, children: [_jsxs(Text, { color: palette.brand, children: ["/", c.name.padEnd(14)] }), _jsx(Text, { color: palette.textMuted, children: c.description })] }, `c-${c.name}`));
        }
    }
    // Shortcut block always visible — append after command lines.
    lines.push(_jsx(Box, { marginTop: 1, children: _jsx(Text, { color: palette.accent, bold: true, children: "Keyboard" }) }, "kb-h"));
    for (const kb of KEYBOARD_SHORTCUTS) {
        lines.push(_jsxs(Box, { gap: 2, children: [_jsx(Text, { color: palette.accent, children: kb.key.padEnd(14) }), _jsx(Text, { color: palette.textMuted, children: kb.desc })] }, `kb-${kb.key}`));
    }
    const maxVisible = Math.max(5, rows - 10);
    const visible = lines.slice(scroll, scroll + maxVisible);
    return (_jsxs(Box, { flexDirection: "column", borderStyle: "round", borderColor: palette.accent, paddingX: 1, width: width, children: [_jsxs(Box, { gap: 1, justifyContent: "space-between", children: [_jsxs(Box, { gap: 1, children: [_jsx(Text, { color: palette.accent, bold: true, children: "help" }), _jsxs(Text, { color: palette.textMuted, dimColor: true, children: [filtered.length, " command", filtered.length === 1 ? '' : 's', filter !== '' ? ` · filter: "${filter}"` : ''] })] }), _jsx(Text, { color: palette.textMuted, dimColor: true, children: "type to filter \u00B7 \u2191\u2193 \u00B7 esc" })] }), visible.length === 0
                ? _jsxs(Text, { color: palette.textMuted, dimColor: true, children: ["No commands match \"", filter, "\""] })
                : _jsx(_Fragment, { children: visible }), lines.length > maxVisible && (_jsx(Box, { marginTop: 1, children: _jsx(Text, { color: palette.textMuted, dimColor: true, children: scroll + maxVisible < lines.length
