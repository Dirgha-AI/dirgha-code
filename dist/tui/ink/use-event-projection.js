...
export function useEventProjection(events, opts = {}) {
    ...
    React.useEffect(() => {
        ...
        function upsertText(prev, id, content) {
            if (!Array.isArray(prev))
                return prev;
            const idx = prev.findIndex((it) => it.kind === "text" && it.id === id);
            if (idx !== -1) {
                const next = [...prev];
                next[idx] = { kind: "text", id, content };
                return next;
            }
            return [...prev, { kind: "text", id, content }];
        }
        ...
        const unsubscribe = events.subscribe((event) => {
            switch (event.type) {
                ...
                case "text_start":
                    ...
                    if (opts.streamingRef)
                        opts.streamingRef.current.active = true;
                    return;
                case "text_delta": {
                    ...
                    if (!flushTimerRef.current) {
                        flushTimerRef.current = setTimeout(() => {
                            ...
                            if (flushTarget.content.length > MAX_LIVE_CHUNK_CHARS &&
                                opts.onCommitSplit) {
                ...
                case "text_end":
                    ...
                    currentTextId = null;
                    if (opts.streamingRef)
                        opts.streamingRef.current.active = false;
                    return;
                case "thinking_start":
                ...
                case "toolcall_end": {
                    ...
                    pendingToolUpdatesRef.current.push((prev) => prev.map((it) => it.kind === "tool" && it.id === endId && it.status === "pending"
                        ? { ...it, argSummary }
                        : it));
                    scheduleToolFlush();
                    return;
                ...
                case "tool_exec_start": {
                    ...
                    pendingToolUpdatesRef.current.push((prev) => {
                        const idx = prev.findIndex((it) => it.kind === "tool" && it.id === event.id);
                        if (idx !== -1) {
                            const copy = [...prev];
                ...
                case "tool_exec_progress": {
                    ...
                    if (!progressTimerRef.current.has(toolId)) {
                        progressTimerRef.current.set(toolId, setTimeout(() => {
                            ...
                            const exists = liveItemsRef.current.some((it) => it.kind === "tool" &&
                                it.id === toolId &&
                                it.status === "running");
                            if (!exists)
                                return;
    ...
    const commitLive = React.useCallback(() => {
        ...
        if (pendingToolUpdates.length > 0) {
            liveItemsRef.current = pendingToolUpdates.reduce((acc, fn) => fn(acc), liveItemsRef.current);
        }
        // Flush any accumulated-but-not-yet-flushed text into liveItemsRef.
        const pt = pendingTextRef.current;
        if (pt) {
            pendingTextRef.current = null;
            const idx = liveItemsRef.current.findIndex((it) => it.kind === "text" && it.id === pt.id);
            if (idx !== -1) {
        ...
        if (pk) {
            pendingThinkingRef.current = null;
            const idx = liveItemsRef.current.findIndex((it) => it.kind === "thinking" && it.id === pk.id);
            if (idx !== -1) {
                liveItemsRef.current[idx] = {
                    kind: "thinking",
                    id: pk.id,
