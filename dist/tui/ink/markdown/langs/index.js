        ...
        if (chunk.startsWith('<')) {
            ...
            if (tagM) {
                ...
                while ((am = attrRe.exec(rest)) !== null) {
                    if (am.index > lastIdx)
                        out.push({ kind: 'plain', value: rest.slice(lastIdx, am.index) });
                ...
                if (lastIdx < rest.length)
                    out.push({ kind: 'plain', value: rest.slice(lastIdx) });
            }
            else {
...
function tokenizeGeneric(src, spec) {
    const out = [];
    ...
    while (i < len) {
        ...
        if (IDENT_START.test(ch)) {
            let j = i + 1;
            while (j < len && IDENT_CONT.test(src[j]))
                j += 1;
            ...
            if (spec.keywords.has(word))
                kind = 'keyword';
            else if (spec.types?.has(word))
                kind = 'type';
            else if (spec.builtins?.has(word))
                kind = 'builtin';
            else if (spec.capitalIsType && /^[A-Z]/.test(word) && j < len && (src[j] === '<' || word.length > 1))
                kind = 'type';
            else
                kind = 'plain';
        ...
        if (spec.operatorChars && spec.operatorChars.includes(ch)) {
            flushPlain();
            let j = i;
            while (j < len && spec.operatorChars.includes(src[j]))
                j += 1;
