 ...
 */
import { createTask, updateTask, listTasks, deleteTask, taskSummary, } from "../context/tasks.js";
const VALID_STATUS = ["pending", "in_progress", "done", "blocked", "cancelled"];
...
export const taskUpdateTool = {
    ...
    async execute(rawInput, _ctx) {
        ...
        const updated = await updateTask(input.id, changes);
        if (!updated)
...
export const taskListTool = {
    ...
    async execute(rawInput, _ctx) {
        const input = rawInput;
        const tasks = await listTasks(input.status);
        if (tasks.length === 0)
            return { isError: false, content: "No tasks found." };
