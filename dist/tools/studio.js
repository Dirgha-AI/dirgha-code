...
async function gatewayOrDirect(gatewayPath, gatewayBody, directFn) {
...
async function generateNvidiaImage(input, model) {
    const prompt = input.style ? `${input.prompt}, style: ${input.style}` : input.prompt;
    // Try gateway first (paid users on our billing)
    const gwResult = await gatewayOrDirect("/api/studio/generate", { prompt, modelId: model, negativePrompt: input.negativePrompt }, async () => {
        throw new Error("gateway_fallback"); // trigger fallback to BYOK
    }).catch(() => null);
    if (gwResult && gwResult.image) {
        return { isError: false, content: `Image generated: ${gwResult.image}` };
    }
    // Fallback: BYOK — call NVIDIA directly
    const key = process.env.NVIDIA_API_KEY;
    if (!key)
        return { isError: true, content: "No gateway session and NVIDIA_API_KEY not set. Run `dirgha login` or `dirgha keys set NVIDIA_API_KEY`" };
    ...
    if (!b64)
        return { isError: true, content: "NVIDIA returned empty response." };
    const { writeFileSync: wfs } = await import("node:fs");
    const { join: pjoin } = await import("node:path");
    const { tmpdir } = await import("node:os");
    const { randomUUID } = await import("node:crypto");
    const path = pjoin(tmpdir(), `dirgha-image-${randomUUID().slice(0, 8)}.png`);
    wfs(path, Buffer.from(b64, "base64"));
    return { isError: false, content: `Image generated (${model}): ${path}` };
}
async function generateOpenRouterImage(input, modelId) {
    const key = process.env.OPENROUTER_API_KEY;
    if (!key)
        return { isError: true, content: "OPENROUTER_API_KEY not set." };
    const prompt = input.style ? `${input.prompt}, style: ${input.style}` : input.prompt;
    ...
    if (!imgUrl)
        return { isError: true, content: "No image returned by model." };
    ...
    async execute(rawInput, _ctx) {
        ...
        try {
            ...
            if (stats.size > 25_000_000)
                return { isError: true, content: "File too large (max 25MB)." };
        }
        catch {
            return { isError: true, content: "File not found." };
        }
        ...
        if (provider === "groq") {
            ...
            if (!key)
                return { isError: true, content: "GROQ_API_KEY not set. Free tier available at groq.com." };
            try {
                ...
                if (!res.ok)
                    return { isError: true, content: `Groq STT ${res.status}` };
                const data = (await res.json());
        ...
        if (!key)
            return { isError: true, content: "OPENAI_API_KEY not set." };
        try {
            ...
            if (!res.ok)
                return { isError: true, content: `OpenAI STT ${res.status}` };
            const data = (await res.json());
            return { isError: false, content: data.text ?? "(no transcription)" };
        }
        catch (err) {
    ...
    async execute(rawInput, _ctx) {
        ...
        if (provider === "openai") {
            ...
            try {
                ...
                if (!res.ok)
                    return { isError: true, content: `OpenAI TTS ${res.status}` };
...
No API key, no internet needed after first download.`,
            ...
            const pipeline = await kokoro.createTTS(modelId, { dtype: "q8" });
            const voiceId = input.voice ?? "af_bella";
            const audio = await pipeline.generate(input.text, { voice: voiceId });
            // Save as WAV
