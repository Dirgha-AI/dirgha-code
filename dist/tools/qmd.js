...
const _platform = `${process.platform}-${process.arch}`;
const _vendoredQmdBin = join(_pkgRoot, "vendor", "qmd", _platform, process.platform === "win32" ? "qmd.exe" : "qmd");
async function resolveQmd() {
    try {
...
export const qmdTool = {
    name: "qmd_search",
    description: "Semantic search over markdown document collections using qmd (Query Markdown Documents). " +
    ...
    inputSchema: {
        ...
        properties: {
            ...
            collection: {
                type: "string",
                description: "Path to the markdown collection directory, or a named qmd collection. " +
    ...
    async execute(raw, ctx) {
        const input = raw;
        const n = input.n ?? 5;
        const collection = input.collection ?? ctx?.cwd ?? process.cwd();
        const qmdBin = await resolveQmd();
        if (!qmdBin) {
            return {
                content: "qmd is not available. Options:\n" +
        ...
        try {
            const args = ["query", input.query, "-n", String(n)];
            if (collection)
