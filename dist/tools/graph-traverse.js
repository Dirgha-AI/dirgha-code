...
export const graphTraverseTool = {
    name: "graph_traverse",
    description: "Recursive forward traversal from a starting node. Returns each " +
    ...
    async execute(rawInput, _ctx) {
        ...
        `;
            ...
            const summary = rows.length === 0
                ...
                : rows
                    .map((p) => `${"  ".repeat(p.depth)}${p.depth === 0 ? "●" : "→"} ${p.node_id}` +
                    `${p.type ? ` [${p.type}]` : ""}` +
            ...
            return {
                content: `graph_traverse failed: ${err.message}`,
                isError: true,
            };
        }
    },
};
//# sourceMappingURL=graph-traverse.js.map