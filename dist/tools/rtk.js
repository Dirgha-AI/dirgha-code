...
const _vendoredBin = join(_pkgRoot, "vendor", "rtk", _platform, process.platform === "win32" ? "rtk.exe" : "rtk");
// Resolve which rtk binary to use: vendored → PATH → null.
async function resolveRtk() {
    ...
    try {
        await execFileAsync(process.platform === "win32" ? "where" : "which", ["rtk"], { timeout: 2000 });
        return "rtk";
    }
    catch {
        return null;
...
export const rtkTool = {
    name: "rtk",
    description: "Run a shell command through rtk (Rust Token Killer) for token-compressed output. " +
    ...
    inputSchema: {
        ...
        properties: {
            command: {
                type: "string",
                description: "The shell command to run (e.g. 'git status', 'cargo test', 'ls -la'). " +
