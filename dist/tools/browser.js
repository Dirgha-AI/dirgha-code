...
async function ensureBrowser() {
    ...
    try {
        ...
        throw new Error(`playwright is not available. Install with: npm install -g playwright && playwright install chromium. Underlying: ${err.message}`);
    }
...
async function doContent(_input) {
    const p = await ensureBrowser();
...
export const browserTool = {
    ...
    async execute(rawInput, _ctx) {
        ...
        try {
            ...
            return fail("Browser tool unavailable: Playwright is not installed. Run: npm install -g playwright && playwright install chromium");
        }
        try {
            switch (input.action) {
                case "goto":
