...
export async function runSetup(opts = {}) {
    ...
    const ask = (question, defaultValue) => new Promise((resolve) => {
        const suffix = defaultValue !== undefined && defaultValue.length > 0
    ...
    const collectedKeys = { ...existingEnv };
    for (const provider of PROVIDERS) {
        ...
        if (answer)
            collectedKeys[provider.env] = answer;
    }
...
function resolveModel(selection, suggestions, fallback) {
    if (/^\d+$/.test(selection)) {
...
async function writeEnvFile(path, values) {
