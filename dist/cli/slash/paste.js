...
function readClipboardImage() {
    ...
    if (plat === "darwin") {
        ...
        return {
            ok: false,
            error: "macOS: install `pngpaste` (brew install pngpaste) to paste images. Text paste needs `pbpaste`.",
        };
    }
    ...
    if (plat === "linux") {
        ...
        return {
            ok: false,
            error: "Linux: install `wl-paste` (Wayland: wl-clipboard pkg) or `xclip` (X11). For image paste both must be present.",
        };
    }
    ...
    if (plat === "win32") {
        ...
        return {
            ok: false,
            error: "Windows: PowerShell clipboard read failed. Make sure something is copied first.",
        };
    }
 ...
 * src/cli/main.ts (or downstream) can prepend the image_url part on
 * the next user turn. Idempotent — overwritten on each call.
 */
function recordPending(result) {
