...
export const gpuSlashCommand = {
    ...
    async execute(args, _ctx) {
        ...
        if (sub === "list") {
            ...
            const result = await gpuListTool.execute({}, {});
        ...
        if (sub === "budget") {
            if (args[1]) {
                ...
                if (isNaN(amt) || amt < 0)
                    return "Usage: /gpu budget <amount>  (0 = unlimited)";
                setBudget(amt);
        ...
        if (sub === "jobs") {
            ...
            if (jobs.length === 0)
                return "No GPU jobs.";
            return jobs.map((j) => `${j.status === "running" ? "🔄" : "✅"} [${j.id}] ${j.gpuType} on ${j.provider} — $${j.totalCost.toFixed(4)} (${j.status})`).join("\n");
        }
        if (sub === "audit") {
            ...
            return entries.map((e) => {
                try {
                    ...
                    return `${p.ts?.slice(0, 19) ?? "?"}  ${p.type}  ${p.job?.gpuType ?? p.jobId ?? ""}`;
                }
                catch {
