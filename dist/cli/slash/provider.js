...
const PROVIDER_LABELS = {
    ...
    ollama: { label: 'Ollama (local)', env: null, blurb: 'Privacy-first · zero cost · zero data leaves' },
    llamacpp: { label: 'llama.cpp (local)', env: null, blurb: 'Privacy-first · zero cost · zero data leaves' },
...
function isConfigured(id) {
    ...
    if (!meta)
        return false;
    if (meta.env === null)
        return true; // local: always counts as available
    return !!process.env[meta.env];
}
async function formatAdd(name) {
