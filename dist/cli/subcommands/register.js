    ...
    for (const rawLine of lines) {
        ...
        if (trimmed.startsWith('- ')) {
            ...
            if (top.arr !== undefined) {
                ...
                const arr = [parseScalar(value)];
                const parentObj = top.obj;
                const newFrame = { indent, obj: parentObj, key: top.key, arr };
...
export const registerSubcommand = {
    ...
    async run(argv, ctx) {
        ...
        try {
            ...
            stderr.write(`${style(defaultTheme.danger, '✗')} Cannot read manifest at ${flags.manifestPath}\n` +
        ...
        try {
            ...
            stderr.write(`${style(defaultTheme.danger, '✗')} Failed to parse YAML: ${e.message}\n`);
            return 1;
        }
        // 3. Validate
        ...
        if (!token) {
            ...
            if (!stored) {
                stderr.write(`${style(defaultTheme.danger, '✗')} Auth required. Run ${style(defaultTheme.accent, 'dirgha auth login')} first.\n`);
                return 1;
            }
            token = stored.token;
        }
        ...
        try {
            ...
            stderr.write(`${style(defaultTheme.danger, '✗')} Network error: ${e.message}\n`);
            return 1;
        }
        ...
        if (res.status === 404 && flags.update) {
            stderr.write(`${style(defaultTheme.danger, '✗')} Agent '${manifest.metadata.name}' not found. ` +
                `Omit --update to register a new agent.\n`);
            return 1;
        }
        if (res.status === 409) {
            stderr.write(`${style(defaultTheme.danger, '✗')} Agent '${manifest.metadata.name}' already registered. ` +
