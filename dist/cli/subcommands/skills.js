...
export const skillsSubcommand = {
    ...
    async run(argv) {
        ...
        if (op === 'show') {
            ...
            if (!hit) {
                stderr.write(`Skill "${name}" not found. List with: dirgha skills\n`);
                return 1;
            }
            stdout.write(`# ${hit.meta.name}\n`);
            stdout.write(`description: ${hit.meta.description}\n`);
            if (hit.meta.version)
                stdout.write(`version:     ${hit.meta.version}\n`);
            if (hit.meta.platforms)
                stdout.write(`platforms:   ${hit.meta.platforms.join(', ')}\n`);
            if (hit.meta.triggers)
                stdout.write(`triggers:    ${JSON.stringify(hit.meta.triggers)}\n`);
        ...
        if (op === 'install') {
            const url = argv[1];
            if (!url) {
                stderr.write(`Missing git URL or directory path.\n${HELP}\n`);
                return 2;
            }
            ...
            if (url.startsWith('-')) {
                stderr.write(`Refusing URL that starts with "-": ${url}\n`);
                return 2;
            }
            const name = argv[2] ?? deriveName(url);
            if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
                stderr.write(`Invalid skill name "${name}". Use [a-zA-Z0-9_-]+.\n`);
                return 2;
            }
            ...
            if (await dirExists(target)) {
                stderr.write(`Skill "${name}" already installed at ${target}. Uninstall first.\n`);
                return 1;
            }
            ...
            if (scan.verdict === 'block') {
                ...
                for (const f of scan.findings.slice(0, 5)) {
                    ...
                    if (f.snippet)
                        stderr.write(style(defaultTheme.muted, `    > ${f.snippet}\n`));
                }
        ...
        if (op === 'uninstall') {
            ...
            if (!await dirExists(target)) {
                stderr.write(`No skill at ${target}.\n`);
                return 1;
            }
            await rm(target, { recursive: true, force: true });
