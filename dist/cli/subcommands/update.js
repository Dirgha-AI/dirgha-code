...
export async function checkLatestVersion(opts) {
    ...
    try {
        ...
        if (!latest)
            throw new Error('registry response had no version field');
...
export function compareSemver(a, b) {
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (ap.num > bp.num)
            return 1;
        if (ap.pre === undefined && bp.pre === undefined)
            continue;
        if (ap.pre === undefined)
            return 1; // bare release > pre-release
        if (bp.pre === undefined)
            return -1;
        if (ap.pre < bp.pre)
            return -1;
...
export function listInstalledPacks(opts = {}) {
...
async function runUpgradePackages(yes) {
    ...
    for (const p of packs)
        stdout.write(`  ${p.name}${p.version ? `  (${p.version})` : ''}\n`);
    ...
    for (const p of packs) {
        ...
        if (reset.status === 0)
            ...
            stderr.write(style(defaultTheme.danger, `✗ ${p.name}: git pull failed (exit ${reset.status})\n`));
    }
...
async function runUpgradeSelf(yes) {
...
export const updateSubcommand = {
    ...
    async run(argv) {
        ...
        if (onlyPacks)
            return runUpgradePackages(yes);
        if (onlySelf)
            return runUpgradeSelf(yes);
        // Default: self + packs.
