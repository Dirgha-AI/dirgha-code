...
function checkTerminal() {
    ...
    const isModern = process.env["WT_SESSION"] ||
    ...
    try {
        ...
        if (installed.length === 0) {
            return {
                name: "lsp",
                status: "warn",
                detail: "no language servers found — LSP tools will return errors (install typescript-language-server, pyright, rust-analyzer, etc.)",
            };
        }
        return {
            name: "lsp",
            status: "pass",
            detail: `${installed.length} server(s): ${installed.map((s) => s.id).join(", ")}`,
        };
    }
    catch {
...
async function checkAllProviderAuth() {
    const results = [];
