...
import { ledgerScope, appendLedger, readLedger, searchLedger, searchLedgerRanked, readDigest, writeDigest } from '../../context/ledger.js';
import { style, defaultTheme } from '../../tui/theme.js';
const VALID_KINDS = ['goal', 'decision', 'observation', 'experiment', 'metric', 'note', 'compaction'];
const HELP = [
    'Usage:',
    ...
    async run(argv) {
        ...
        if (op === 'add') {
            ...
            if (!VALID_KINDS.includes(kind)) {
                stderr.write(`Bad kind. Must be one of: ${VALID_KINDS.join(', ')}\n`);
                return 2;
            }
            const text = rest.slice(2).join(' ').trim();
            if (!text) {
                stderr.write('Missing text.\n' + HELP + '\n');
                return 2;
            }
        ...
        if (op === 'search') {
            ...
            if (exact) {
                ...
                if (hits.length === 0) {
                    stdout.write(`No entries match "${q}" in scope ${scopeName}.\n`);
                    return 0;
                }
                for (const e of hits)
                    stdout.write(`  ${e.ts}  ${e.kind}  ${e.text}\n`);
            }
            else {
                ...
                for (const r of ranked)
                    stdout.write(`  ${r.score.toFixed(2)}  ${r.entry.ts}  ${r.entry.kind}  ${r.entry.text}\n`);
            }
            return 0;
        }
        if (op === 'show') {
