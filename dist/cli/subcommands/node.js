...
// Subcommand: start
// ---------------------------------------------------------------------------
async function runStart(detach = false) {
    if (detach) {
    ...
    while (true) {
        ...
        stdout.write(`[${formatTime()}] ${style(defaultTheme.success, `+${result.credits_earned} credits`)}` +
...
export const nodeSubcommand = {
    ...
    async run(argv) {
        ...
        if (op === 'start')
            return runStart(argv.includes('--detach'));
        if (op === 'stop')
            return runStop();
        if (op === 'status')
            return runStatus();
