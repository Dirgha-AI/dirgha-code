...
function compactMessage(message, olderThanIdx, myIdx) {
    // Strip verbose tool_result/thinking content from older messages.
    if (myIdx >= olderThanIdx)
        return message;
    if (typeof message.content === 'string')
        return message;
    const pruned = message.content.map(part => {
        if (part.type === 'thinking')
            return { type: 'text', text: '[thinking elided]' };
        if (part.type === 'tool_result') {
...
export const compactSubcommand = {
    ...
    async run(argv) {
        ...
        entries.forEach((e, i) => { if (e.type === 'message')
