...
async function ensureKbRoot() {
    ...
    if (await stat(cfg).then(() => true).catch(() => false))
        return;
    ...
    async run(argv) {
        ...
        if (op === 'ingest') {
            ...
            for (const src of sources) {
                ...
                if (!exists) {
                    stdout.write(style(defaultTheme.muted, `  skip ${src} (not present)\n`));
                    continue;
                }
                ...
                if (r.status === 0)
                    ...
                    stderr.write(style(defaultTheme.danger, `  ✗ openkb add ${src} exited ${r.status}\n`));
            }
        ...
        if (op === 'list')
            return passthrough(argv);
        if (op === '--')
            return passthrough(argv.slice(1));
