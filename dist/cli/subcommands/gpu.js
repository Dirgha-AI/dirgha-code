...
export const gpuSubcommand = {
    ...
    async run(argv, _ctx) {
        ...
        if (sub === "status" || sub === "s") {
            const id = argv[1];
            if (!id) {
                console.log("Usage: dirgha gpu status <instance-id>");
                return 1;
            }
        ...
        if (sub === "destroy" || sub === "d") {
            const id = argv[1];
            if (!id) {
                console.log("Usage: dirgha gpu destroy <instance-id>");
                return 1;
            }
            for (const p of getAllProviders()) {
                try {
        ...
        if (sub === "audit") {
            ...
            for (const e of entries) {
                try {
                    ...
                    console.log(`  ${(p.ts ?? "").slice(0, 19)}  ${p.type}  ${p.job?.gpuType ?? p.jobId ?? ""}`);
                }
                catch {
        ...
        if (sub === "market") {
            ...
            if (marketSub === "list" || marketSub === "ls") {
                ...
                if (listings.length === 0) {
                    console.log("No marketplace listings.");
                    return 0;
                }
            ...
            if (marketSub === "post") {
                ...
                if (!gpuType || !price) {
                    console.log("Usage: dirgha gpu market post <gpu-type> <price-per-hr>");
                    return 1;
                }
