...
function recordAudio(recorder, outPath, durationSec) {
    try {
        if (recorder === "arecord") {
...
export const voiceSubcommand = {
    name: "voice",
    description: "Record speech from microphone and transcribe it",
    async run(argv, _ctx) {
        const recorder = detectRecorder();
        if (!recorder) {
            process.stderr.write("❌ No audio recorder found.\n" +
                "Install one:\n" +
        ...
        if (!process.env.GROQ_API_KEY && !process.env.OPENAI_API_KEY) {
            process.stderr.write("❌ No STT API key found.\n" +
                "Set one:\n" +
