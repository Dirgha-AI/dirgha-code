...
async function main() {
    ...
    const isBareInvocation = rawArgs.length === 0 ||
        rawArgs.every((a) => a.startsWith("-") &&
            a !== "--help" &&
            a !== "-h" &&
            a !== "--version" &&
            a !== "-V");
    ...
    // first occurrence of the verb in argv.
    {
        ...
        if (cmd) {
            ...
            try {
                ...
                process.stderr.write(`[${verb}] ${err instanceof Error ? err.message : String(err)}\n`);
            }
    ...
    if (typeof flags.mode === "string" &&
        ...
        process.env["DIRGHA_MODE"] = flags.mode;
    }
    ...
    const mcpShutdownOnSignal = (code) => {
        Promise.race([
            mcpShutdown(),
    ...
    const compactionTransform = matched.length > 0
        ? async (msgs) => injectSkills(await baseCompactionTransform(msgs), matched)
...
function readAllStdin() {
    return new Promise((resolve, reject) => {
        ...
        let total = 0;
        let capped = false;
        const timer = setTimeout(() => reject(new Error("stdin read timed out")), STDIN_TIMEOUT_MS);
