...
async function cmdUp(tasks, _opts) {
    if (tasks.length === 0) {
    ...
    for (const r of results) {
        if (r) {
            ...
            r.done.then((code) => {
                ...
                const hasRunning = registry.get(session.id)?.agents.some((a) => a.status === "running") ?? false;
...
async function cmdAttach(args, _opts) {
    ...
    for (const agent of session.agents) {
        stdout.write(`  ${agent.id.padEnd(16)} ${agent.status.padEnd(10)} ` +
...
async function cmdKill(args, _opts) {
    const target = args[0];
    if (!target) {
    ...
    for (const session of registry.list()) {
        const agent = session.agents.find((a) => a.id === target || a.label === target);
        if (agent) {
...
async function cmdDown(args, _opts) {
    const id = args[0];
    if (!id) {
...
async function cmdLog(args, _opts) {
    const id = args[0];
    if (!id) {
    ...
    for (const entry of entries.slice(-100)) {
        stdout.write(`[${entry.timestamp}] ${entry.agentId} ${entry.event} ` +
