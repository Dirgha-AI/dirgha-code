...
- You have a **per-session SQLite workspace** with relational, FTS, vector (`kb_search`), and graph (`graph_neighbors`, `graph_traverse`) shapes, plus a `transactional_write` tool to commit across all four atomically. Inspect the workspace with `db_workspace_info` before a multi-model write so you know what's loaded.
