...
export function loadVecExtension(db) {
    ...
    try {
        ...
        process.stderr.write(`[Dirgha] sqlite-vec extension failed to load (vector search disabled): ${err.message}\n`);
    }
}
/**
 ...
 */
export function migrateVecSchema(db) {
    try {
    ...
    `);
      ...
      `);
            ...
            process.stderr.write(`[Dirgha] vec virtual table creation failed (vector search disabled): ${err.message}\n`);
        }
    }
    catch (err) {
        recordDbError(err);
 ...
 */
export function isVecLoaded(db) {
    try {
        const row = db
            .prepare("SELECT vec_version() AS version")
            .get();
