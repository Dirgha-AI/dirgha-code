...
- [x] **Tool-output truncation** in `ToolItem` — results capped at 40 chars inline
...
- [x] **Paste-burst detector** — `src/tui/components/InputHooks.ts` coalesces rapid keystrokes <8ms apart into a single paste buffer. Embedded `\n` in a burst are text, not submit.
