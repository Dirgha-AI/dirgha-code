...
This plugin depends on the extensions API hooks `before_skill_install` and `before_extension_load`. As of 1.4.0 the API supports `before_skill_install` only via the existing `loadExtensions` flow + `api.on(event, handler)`. The two new hook events need to be emitted from:
