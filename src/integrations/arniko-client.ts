...
export function createArnikoClient(opts: ArnikoClientOptions = {}) {
  const baseUrl =
    opts.baseUrl ?? process.env.DIRGHA_ARNIKO_URL ?? "http://localhost:3010";
  ...
  return {
    ...
    async bootstrap(
      ...
      return new Promise((resolveBootstrap) => {
        ...
        child.on("error", (err) =>
          resolveBootstrap({ started: false, message: err.message }),
        );
        child.on("exit", (code) =>
          resolveBootstrap({
            started: code === 0,
            message:
              code === 0
                ...
                : Buffer.concat(
                    errChunks as unknown as readonly Uint8Array[],
                  ).toString("utf8"),
          }),
        );
      });
