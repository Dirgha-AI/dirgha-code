...
import type { Provider } from '../providers/iface.js';
...
export async function runParity(scenarios: ParityScenario[]): Promise<ParityReport> {
  const results: ParityScenarioReport[] = [];
...
async function runScenario(scenario: ParityScenario): Promise<ParityScenarioReport> {
...
function makeProvider(id: ParityScenario['provider'], baseUrl: string): Provider {
  switch (id) {
    case 'nvidia':
...
function diffSequences(expected: readonly string[], actual: readonly string[]): string {
