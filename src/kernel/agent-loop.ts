    ...
    for (let turnIndex = 0; turnIndex < maxTurns; turnIndex++) {
      ...
      try {
        ...
        const isModelNotFound =
          /not a valid model id|deprecated|model_not_found/i.test(errMsg);
        if (isModelNotFound) {
          ...
          if (_fallbackIndex < cfg.fallbackModels.length) {
            ...
            if (fbEntry.model !== cfg.model) {
              try {
                ...
                // use the new backend.
                (cfg as { provider: Provider; model: string }).provider = fbProvider;
                (cfg as { provider: Provider; model: string }).model = fbEntry.model;
