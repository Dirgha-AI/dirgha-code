...
export async function appendLedger(
  ...
  try {
    ...
    /* swallow — ledger writes never break the run */
  }
}
...
export async function readLedger(
  ...
  const start =
    limit !== undefined && lines.length > limit ? lines.length - limit : 0;
  const out: LedgerEntry[] = [];
  for (let i = start; i < lines.length; i++) {
    try {
      out.push(JSON.parse(lines[i]) as LedgerEntry);
    } catch {
      /* skip malformed */
    }
  }
...
function cosineRank(
  query: string,
  entries: LedgerEntry[],
): Array<{ entry: LedgerEntry; score: number }> {
 ...
 * queries like "ok".
 */
export async function searchLedgerRanked(
  scope: LedgerScope,
  query: string,
  opts: { topK?: number } = {},
): Promise<Array<{ entry: LedgerEntry; score: number }>> {
...
export interface LedgerStore {
  ...
  render(opts?: { tailEntries?: number }): Promise<string>;
}
...
export function createLedgerStore(
  ...
  const scope = ledgerScope(name, home);
  return {
    scope,
    append: (entry) => appendLedger(scope, entry),
    read: (limit?) => readLedger(scope, limit),
    search: (query) => searchLedger(scope, query),
    render: (opts?) => renderLedgerContext(scope, opts),
  };
}

export async function renderLedgerContext(
  scope: LedgerScope,
  opts: { tailEntries?: number } = {},
): Promise<string> {
  ...
  if (digest.trim()) {
    sections.push(
      `<ledger_digest scope="${scope.name}">\n${digest.trim()}\n</ledger_digest>`,
    );
  }
  if (tail.length > 0) {
    const tailLines = tail
      .map((e) => `[${e.ts}] (${e.kind}) ${e.text}`)
      .join("\n");
    sections.push(
      `<ledger_tail scope="${scope.name}" count="${tail.length}">\n${tailLines}\n</ledger_tail>`,
    );
  }
