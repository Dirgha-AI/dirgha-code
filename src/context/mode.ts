...
async function withConfigLock<T>(fn: () => Promise<T>): Promise<T> {
  ...
  throw new Error(
    "config lock timeout after 500ms — another session may be stuck",
  );
}
...
export async function resolveMode(): Promise<Mode> {
  ...
  if (fromEnv && (MODES as readonly string[]).includes(fromEnv))
    return fromEnv as Mode;
  const cfg = await readStoredConfig();
  if (cfg.mode && (MODES as readonly string[]).includes(cfg.mode))
    return cfg.mode;
  return DEFAULT_MODE;
}
