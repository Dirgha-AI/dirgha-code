...
export async function maybeCompact(
  ...
  if (!finalSummary) {
    ...
    const tokensAfterFallback = trimmed.reduce(
      (acc, m) => acc + estimateTokens(flatten(m)),
      0,
    );
    return {
...
function truncateLargeToolOutputs(messages: Message[]): Message[] {
  ...
  return messages.map((msg): Message => {
    ...
    const anyLarge = msg.content.some(
      (p) =>
        (p as { type: string }).type === "tool_result" &&
        typeof (p as { type: string; content?: unknown }).content === "string" &&
        ((p as { type: string; content: string }).content.length > MAX_TOOL_RESULT_CHARS),
    );
    if (!anyLarge) return msg;
    return {
      ...msg,
      content: msg.content.map((p) => {
        ...
        const part = p as { type: "tool_result"; toolUseId: string; content: string; isError?: boolean };
        if (part.content.length <= MAX_TOOL_RESULT_CHARS) return p;
...
async function summarise(
  ...
  const verifyMessages: Message[] = [
    ...
    { role: "assistant" as const, content: summary },
    {
      role: "user" as const,
      content:
        "Critically evaluate the <state_snapshot> you just generated. Did you omit any specific technical details, file paths, tool results, or user constraints from the history? If anything is missing or could be more precise, generate a FINAL improved <state_snapshot>. Otherwise, repeat the exact same <state_snapshot> again.",
    },
  ];
  ...
  try {
    ...
    for await (const ev of cfg.summarizer.stream({
      ...
      if (ev.type === "text_delta") verified += ev.delta;
    }
    if (verified.trim()) finalSummary = verified.trim();
  } catch {
...
function renderForSummary(msg: Message, thinkingChars: number): string {
...
export function createCompactionTransform(opts: {
  ...
  session?: Session;
  hooks?: HookRegistry;
  history: Message[]; // mutable ref shared with caller
