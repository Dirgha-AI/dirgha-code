...
class JsonRpcSplitter {
  ...
  transform(chunk: string): RpcMessage[] {
    this.buffer += chunk;
...
export function createLspConnection(proc: ChildProcess): LspConnection {
  ...
  function sendRaw(raw: string) {
    ...
    if (proc.stdin) proc.stdin.write(header + raw);
  }
...
export async function createLspClient(
  ...
  const initResult = await Promise.race([
    ...
    new Promise<never>((_, reject) =>
      setTimeout(
        () => reject(new Error(`LSP initialize timeout for ${serverId}`)),
        LSP_INIT_TIMEOUT,
      ),
    ),
  ]);
  ...
  const client: LspClient = {
    ...
    async goToDefinition(pos: Position, filePath: string): Promise<Location[]> {
      await openFile(filePath);
      const result = await connection
        .sendRequest<Location[] | Location | null>("textDocument/definition", {
      ...
      return arr.filter(Boolean).map((loc: any) => ({
        uri: uriToPath(loc.targetUri ?? loc.uri),
        range: (loc.targetRange ?? loc.range) as Location["range"],
      }));
    },

    async findReferences(pos: Position, filePath: string): Promise<Location[]> {
      await openFile(filePath);
      const result = await connection
        .sendRequest<Location[] | null>("textDocument/references", {
          textDocument: { uri: pathToFileURL(filePath).href },
          position: { line: pos.line, character: pos.character },
        ...
        .catch(() => []);
      if (!result) return [];
      return result.filter(Boolean).map((loc: any) => ({
        uri: uriToPath(loc.targetUri ?? loc.uri),
        range: (loc.targetRange ?? loc.range) as Location["range"],
      }));
    },

    async hover(pos: Position, filePath: string): Promise<HoverResult | null> {
      await openFile(filePath);
      return connection
        .sendRequest<HoverResult | null>("textDocument/hover", {
          textDocument: { uri: pathToFileURL(filePath).href },
          position: { line: pos.line, character: pos.character },
        })
        .catch(() => null);
    ...
    async documentSymbols(
      ...
      await openFile(filePath);
      const result = await connection
        .sendRequest<(DocumentSymbol | SymbolInfo)[] | null>(
        ...
        .catch(() => []);
      if (!result) return [];
      return result.map((sym: DocumentSymbol | SymbolInfo) => {
    ...
    async getDiagnostics(): Promise<Record<string, Diagnostic[]>> {
      const result: Record<string, Diagnostic[]> = {};
      for (const [file, diags] of diagnostics.entries()) result[file] = diags;
      return result;
    },

    async shutdown() {
      try {
        await connection.sendRequest<null>("shutdown");
      } catch {
        /* ignore */
      }
