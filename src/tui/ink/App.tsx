...
let _cachedInitialMessages: Message[] | null = null;
...
export function App(props: AppProps): React.JSX.Element {
  ...
  const loopDetectorRef = React.useRef<LoopDetector>(new LoopDetector());
  ...
  const projection = useEventProjection(props.events, {
    ...
    onSessionTitle: React.useCallback((title: string) => {
      ...
      if (s) {
        ...
        const id = s.id;
        if (id) {
          import("../../state/index.js").then(({ renameSession }) =>
            renameSession(id, safe).catch(() => {}),
          );
        }
      }
    }, []),
  ...
  const models = React.useMemo<ModelEntry[]>(
    () => props.models ?? defaultModelCatalogue(),
    [props.models],
  );
        ...
        const ctx: SlashContext = {
          ...
          listSessions: async () => {
            ...
            if (entries.length === 20) {
              lines.push(
                `  (showing 20 — use /session load <id> to restore)`,
              );
            }
            return lines.join("\n");
          },
          ...
          apiBase: () =>
            ...
            "https://api.dirgha.ai",
          upgradeUrl: () =>
            process.env["DIRGHA_UPGRADE_URL"] ??
  ...
  const handleKeySetSave = React.useCallback(
    (value: string): void => {
  ...
  const handleAtPick = React.useCallback(
    (path: string): void => {
  ...
  const ARGLESS_SLASH = React.useMemo(
    () =>
      new Set([
        "help",
        "exit",
  ...
  const handleSlashPick = React.useCallback(
    (name: string): void => {
 ...
 */
function tryReadKeyFromStore(envVar: string): boolean {
  try {
...
function buildProviderEntries(
  ...
  out.sort(
    (a, b) =>
      Number(b.hasKey) - Number(a.hasKey) || a.label.localeCompare(b.label),
  );
  return out;
}
