...
export function DenseToolMessage(props: DenseToolMessageProps): React.JSX.Element {
  ...
  const glyph =
    props.status === 'error' ? TOOL_STATUS.ERROR
    : props.status === 'done' ? TOOL_STATUS.SUCCESS
    : SPINNER[frame];

  const glyphColour =
    props.status === 'error' ? palette.status.error
    : props.status === 'done' ? palette.status.success
    : palette.status.warning;

  const nameColour =
    props.status === 'done' ? palette.text.secondary
    : props.status === 'error' ? palette.status.error
    : palette.text.primary;

  const elapsed = formatElapsed(props.durationMs ?? 0);
  ...
  return (
    <Box paddingLeft={2} flexDirection="row">
      <Box minWidth={2}>
        <Text color={glyphColour} bold={props.status !== 'running'}>{glyph}</Text>
      </Box>
      <Text color={palette.text.accent}>{iconFor(props.name)}</Text>
      <Text>{' '}</Text>
      <Text bold color={nameColour}>{label}</Text>
      ...
      {props.durationMs !== undefined && (
        <>
          <Text>{'  '}</Text>
          <Text color={palette.text.secondary} dimColor>{elapsed}</Text>
        </>
      )}
      {summary && (
        <>
          <Text>{'  '}</Text>
          <Text color={palette.text.secondary} dimColor>{summary}</Text>
        </>
      )}
    </Box>
  );
...
const DENSE_TOOLS = new Set([
  'fs_read', 'fs_ls', 'fs_edit',
  'search_grep', 'search_glob',
  'git', 'git_commit',
]);
