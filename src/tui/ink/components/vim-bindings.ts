 ...
 */
export function applyVimKey(value: string, state: VimState, ch: string): VimApplyResult {
  ...
  switch (ch) {
    ...
    case 'x': {
      if (state.cursor >= value.length) return { value, state: clearPending, handled: true };
    ...
    case 'p': {
      if (state.register === '') return { value, state: clearPending, handled: true };
