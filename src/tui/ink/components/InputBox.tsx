...
export function InputBox(props: InputBoxProps): React.JSX.Element {
  ...
  const historyCacheRef = React.useRef<Map<number, string>>(new Map());
  ...
  useInput(
    (inputCh, key) => {
      ...
      if (key.upArrow && history.length > 0) {
        ...
        if (nextIdx >= history.length) return; // At oldest entry, no-op.
      ...
      if (key.downArrow && historyIdx !== null) {
        ...
        if (nextIdx < -1) return; // Past cache bottom — shouldn't happen.
        if (nextIdx === -1) {
      ...
      if (props.vimMode === true) {
        ...
        if (vimState.mode === "NORMAL" && !key.ctrl && !key.meta) {
          ...
          if (inputCh && inputCh.length === 1) {
            ...
            props.onChange(
              props.value.slice(0, pos) + inputCh + props.value.slice(pos),
            );
          }
