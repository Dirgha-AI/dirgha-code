...
interface HealthState {
  ...
  lastSuccess: number;
  avgLatencyMs: number;
  blacklistedUntil: number | null;          // cooldown expiry – mirrors cooldownUntil in public API
  cooldownLevel: number;
...
const COOLDOWN_BACKOFF_MS: readonly number[] = [
...
function toKey(provider: string, model: string): HealthKey {
  return `${provider}:${model}` as HealthKey;
}
...
function ensureState(provider: string, model: string): HealthState {
...
function stateToObject(): Record<HealthKey, Record<string, unknown>> {
  const obj: Record<HealthKey, Record<string, unknown>> = {};
  ...
  try {
    ...
    if (parsed.entries) {
      for (const [_key, entry] of Object.entries(parsed.entries)) {
        ...
        const prov = entry.provider as string;
 ...
 * Persists the updated state to disk.
 */
export function recordFailure(provider: string, model: string, _error: string): void {
  const now = Date.now();
  const s = ensureState(provider, model);
  s.totalRequests++;
  s.failures++;
...
export function isBlacklisted(provider: string, model: string): boolean {
  ...
  if (s.failoverBlacklisted) return true;
...
function stateToPublic(s: HealthState): ProviderHealth {
  const now = Date.now();
