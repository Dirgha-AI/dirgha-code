...
import type { NimModel } from "../providers/nim-catalogue.js";
...
function nimToRemote(m: NimModel): RemoteModelDescriptor {
  return {
    id: m.id,
    label: m.label,
    provider: "nvidia",
...
const PROVIDER_MAP: Array<{ provider: string; catalogue: ModelDescriptor[] }> =
  [
...
function buildLocalCatalogue(): RemoteModelDescriptor[] {
...
export async function fetchRemoteCatalogue(): Promise<RemoteModelDescriptor[]> {
  try {
    ...
    const data = (await resp.json()) as { models?: RemoteModelDescriptor[] };
...
export async function loadCatalogue(): Promise<RemoteModelDescriptor[]> {
  try {
...
export function getCachedCatalogue(): RemoteModelDescriptor[] | null {
  try {
...
function isCacheFresh(cache: ModelsCache): boolean {
...
async function readCache(cachePath: string): Promise<ModelsCache | null> {
  try {
...
async function writeCache(cachePath: string, data: ModelsCache): Promise<void> {
