...
export async function buildTarball(
  ...
  await new Promise<void>((resolveTar, rejectTar) => {
    ...
    child.on("exit", (code) => {
      if (code === 0) resolveTar();
        ...
        rejectTar(
          new Error(
            `tar exited ${code}: ${Buffer.concat(errChunks as readonly Uint8Array[]).toString("utf8")}`,
          ),
        );
    });
  });
  ...
  if (info.size > MAX_SIZE_BYTES) {
    throw new Error(
      `Tarball is ${info.size} bytes; exceeds ${MAX_SIZE_BYTES} byte cap. Add patterns to .dirghaignore.`,
    );
  }
