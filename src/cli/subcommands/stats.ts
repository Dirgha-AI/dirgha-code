...
import { streamJsonl, type SessionEntry } from '../../context/session.js';
...
function windowStart(win: Window): Date | undefined {
  const now = new Date();
  if (win === 'today') { const d = new Date(now); d.setHours(0, 0, 0, 0); return d; }
  if (win === 'week')  { const d = new Date(now); d.setDate(d.getDate() - 7); return d; }
  if (win === 'month') { const d = new Date(now); d.setDate(d.getDate() - 30); return d; }
  return undefined;
}

function costFor(model: string, input: number, output: number, cached: number): number {
...
async function aggregate(win: Window): Promise<Aggregate> {
  const dir = join(homedir(), '.dirgha', 'sessions');
  const allFiles = (await readdir(dir).catch(() => [] as string[])).filter(f => f.endsWith('.jsonl'));
  // Cap to the 100 most-recent files by mtime to prevent hanging on large stores.
  const filesWithMtime = await Promise.all(
    allFiles.map(async f => ({ f, mtime: (await stat(join(dir, f)).catch(() => ({ mtimeMs: 0 }))).mtimeMs }))
  );
  ...
  for (const file of files) {
    ...
    const pushEntry = (entry: SessionEntry): void => {
      const ts = new Date(entry.ts);
      if (since && ts < since) return;
      const dayKey = entry.ts.slice(0, 10);

      if (entry.type === 'message') { agg.messages += 1; return; }
      if (entry.type === 'model_change') { currentModel = entry.to; return; }
      if (entry.type !== 'usage') return;
...
function printTable(win: Window, a: Aggregate): void {
