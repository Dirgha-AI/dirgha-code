...
async function readTurnEnds(): Promise<AuditEntry[]> {
  ...
  for (const line of text.split('\n')) {
    ...
    try {
      ...
      if (e.kind === 'turn-end' && e.model && e.usage) out.push(e);
...
function aggregate(entries: AuditEntry[]): ModelTotal[] {
  const byModel = new Map<string, ModelTotal>();
  for (const e of entries) {
    if (!e.model || !e.usage) continue;
...
function emit(label: string, totals: ModelTotal[], json: boolean): void {
  if (json) {
  ...
  for (const t of totals) {
    totalCost += t.costUsd; totalTurns += t.turns; totalIn += t.inputTokens; totalOut += t.outputTokens;
...
export const costSubcommand: Subcommand = {
  name: 'cost',
  ...
  async run(argv): Promise<number> {
    ...
    if (op === 'day') {
      const day = args[1];
      if (!day || !/^\d{4}-\d{2}-\d{2}$/.test(day)) { stderr.write(`${usage()}\n`); return 1; }
      emit(day, aggregate(all.filter(e => e.ts.startsWith(day))), json);
      return 0;
    }
    if (op === 'week') {
