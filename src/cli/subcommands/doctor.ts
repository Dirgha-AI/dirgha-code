...
async function checkDbTelemetry(): Promise<CheckResult> {
  try {
    ...
    const detail =
      `${data.failedWrites} / ${data.totalWrites} failures` +
      (data.lastError
...
async function checkVecExtension(): Promise<CheckResult> {
  try {
 ...
 */
async function checkKbChunks(): Promise<CheckResult> {
  try {
    const { openDb } = await import("../../state/db.js");
...
export const doctorSubcommand: Subcommand = {
  name: "doctor",
  description:
    "Environment diagnostics (node, git, providers). Pass --strict to gate on remote auth.",
  async run(argv): Promise<number> {
    ...
    const failedLocal = results.some(
      (r) => r.status === "fail" && LOCAL_CHECK_NAMES.has(r.name),
    );
