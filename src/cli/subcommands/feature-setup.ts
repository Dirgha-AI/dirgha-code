...
function httpsDownload(url: string, destPath: string): Promise<number> {
  return new Promise((resolve, reject) => {
    function follow(u: string, redirects: number): void {
 ...
 */
async function extractNodeFromTarGz(tarGzPath: string, outDir: string): Promise<string | null> {
...
async function detectFeatures(): Promise<FeatureStatus> {
...
function getSqliteVersion(): string {
  try {
    const pkg = _require('better-sqlite3/package.json') as { version?: string };
    if (typeof pkg.version === 'string') return pkg.version;
  } catch { /* fall through */ }
  // Read from our own optionalDependencies.
  try {
    const ownPkg = _require('../../package.json') as {
      optionalDependencies?: Record<string, string>;
    };
    const ver = ownPkg.optionalDependencies?.['better-sqlite3'];
    if (ver) return ver.replace(/^[^0-9]*/, '');
  } catch { /* fall through */ }
  return '12.9.0';
}
...
type InstallResult =
  | { ok: true }
  | { ok: false; reason: string; windowsGuide?: boolean };

async function installSqlitePrebuilt(): Promise<InstallResult> {
...
async function installSqliteViaFallback(): Promise<InstallResult> {
...
function printWindowsGuide(): void {
  stdout.write(
    `  ✗ Prebuilt binary not available for your Node version.\n\n` +
...
async function installSqlite(): Promise<boolean> {
  // Step 1: try prebuilt.
...
export async function runFeatureSetup(): Promise<number> {
  ...
  try {
    ...
    const installSqliteNow =
      missingSqlite && (choice === '1' || choice === '2');
