...
async function runTail(json: boolean, last: number, follow: boolean, matchKind: (e: AuditEntry) => boolean = () => true): Promise<number> {
  await ensureLog();
...
export const auditSubcommand: Subcommand = {
  ...
  async run(argv): Promise<number> {
    ...
    if (nIdx >= 0 && nIdx + 1 < argv.length) lastNum = Math.max(1, Number.parseInt(argv[nIdx + 1], 10) || 20);
    else if (lastIdx >= 0 && lastIdx + 1 < argv.length) lastNum = Math.max(1, Number.parseInt(argv[lastIdx + 1], 10) || 20);
    ...
    const cleanArgs = args.filter((_, i, arr) => {
      if (i > 0 && (arr[i - 1] === '-n' || arr[i - 1] === '--last')) return false;
      return true;
    });
    ...
    if (op === 'tail') return runTail(json, lastNum, follow, matchKind);
    if (op === 'search') {
