...
export const PROVIDERS: ProviderEntry[] = [
  ...
  },
  {
    ...
    suggested: ["claude-opus-4-7", "claude-sonnet-4-6", "claude-haiku-4-5"],
  },
  {
    label: "OpenAI",
    env: "OPENAI_API_KEY",
  ...
  },
  {
    ...
    suggested: ["cohere/command-a-03-2025", "cohere/command-r-plus"],
  },
  {
    label: "Cerebras",
    env: "CEREBRAS_API_KEY",
    helpUrl: "https://cloud.cerebras.ai/platform/keys",
    suggested: ["cerebras/llama-3.3-70b", "cerebras/qwen-3-32b"],
  },
  {
    label: "Together AI",
  ...
  },
  {
    ...
    suggested: ["perplexity/sonar", "perplexity/sonar-pro"],
  },
  {
    label: "xAI (Grok)",
    env: "XAI_API_KEY",
    helpUrl: "https://console.x.ai/team/api-keys",
    suggested: ["grok-4-fast", "grok-4"],
  },
  {
    label: "Groq",
    env: "GROQ_API_KEY",
    helpUrl: "https://console.groq.com/keys",
    suggested: ["groq/llama-3.3-70b-versatile", "groq/qwen-3-32b"],
  },
  {
    label: "Z.AI / GLM",
    env: "ZAI_API_KEY",
    helpUrl: "https://docs.z.ai/devpack/tool/openai",
    suggested: ["zai/glm-4.6", "zai/glm-4.5-air"],
  },
];
...
export async function runSetup(opts: SetupOptions = {}): Promise<void> {
  ...
  for (const provider of PROVIDERS) {
    const current =
      existingEnv[provider.env] ?? process.env[provider.env] ?? "";
    print(
      style(defaultTheme.userPrompt, `\n${provider.label}`) +
        style(defaultTheme.muted, `   (${provider.helpUrl})`),
    );
  ...
  const selection = await ask(
    "  choose by number or paste a model id",
  ...
  const showThinkingAns = (
    await ask(
      "\n  Show model thinking when available? (y/N)",
      existing.showThinking ? "y" : "n",
    )
  ).toLowerCase();
  ...
  print(
    `  ${style(defaultTheme.muted, `set -a && source ${envPath} && set +a`)}`,
  );
