...
function detectProvider(id: string): string {
  if (id.endsWith(":free")) return "openrouter";
  if (id.includes("/")) {
    const vendor = id.split("/")[0].toLowerCase();
    if (VENDOR_TO_PROVIDER[vendor]) return VENDOR_TO_PROVIDER[vendor];
...
async function testModel(id: string): Promise<string> {
    ...
    if (args.length > 0) {
      ...
      if (first === "search") {
        ...
        try {
          ...
          const json = await res.json() as { data?: Array<{ id: string; context_length?: number; pricing?: { prompt?: string; completion?: string } }> };
          ...
          const lines: string[] = [];
          lines.push(
            `  ${"ID".padEnd(42)}  ${"Context".padEnd(10)}  ${"Prompt/1M tokens"}`,
          );
          lines.push(
            `  ${"─".repeat(42)}  ${"─".repeat(10)}  ${"─".repeat(16)}`,
          );
          for (const m of filtered) {
            ...
            lines.push(
              `  ${m.id.padEnd(42)}  ${ctxStr.padEnd(10)}  ${priceStr}`,
            );
          }
          return lines.join("\n");
        } catch (err: unknown) {
