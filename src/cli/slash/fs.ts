...
class MountManager {
  ...
  private save(): void {
    try {
      ...
      writeFileSync(
        this.storePath,
        JSON.stringify(this.mounts, null, 2),
        "utf8",
      );
    } catch {
      /* ignore */
  ...
  async mount(type: MountEntry["type"], path: string): Promise<string> {
    const existing = this.mounts.find((m) => m.path === path);
    if (existing) return `Already mounted: ${existing.type} at ${path}`;
    try {
...
export const fsCommand: SlashCommand = {
  name: "fs",
  description:
    "Filesystem mount management: /fs mount | /fs unmount | /fs list",
  async execute(args, _ctx) {
    ...
    if (sub === "mount") {
      const type = args[1] as MountEntry["type"];
      ...
      try {
        ...
        if (type === "r2") return "R2 mount requires bucket configuration";
    ...
    if (sub === "unmount") {
      ...
      if (!mountPoint) return "Usage: /fs unmount <path>";
    ...
    if (sub === "list") {
      ...
      if (mounts.length === 0) return "No active filesystem mounts.";
      return [
        "Filesystem Mounts:",
        ...mounts.map(
          (m) => `  ${m.type.padEnd(10)} ${m.path}  (${m.createdAt})`,
        ),
      ].join("\n");
    }
