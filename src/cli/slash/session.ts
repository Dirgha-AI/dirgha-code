...
export const sessionCommand: SlashCommand = {
  ...
  async execute(args, ctx) {
    const op = args[0];
    if (!op || op === 'list') return ctx.listSessions();
    if (op === 'load' && args[1]) return ctx.loadSession(args[1]);

    if (op === 'rename') {
      const [, oldId, newId] = args;
      if (!oldId || !newId) return `Missing argument.\n${usage()}`;
      const fromPath = sessionPath(oldId);
      const toPath = sessionPath(newId);
      const oldExists = await stat(fromPath).then(() => true).catch(() => false);
      if (!oldExists) return `Session "${oldId}" not found.`;
      const newExists = await stat(toPath).then(() => true).catch(() => false);
      if (newExists) return `Session "${newId}" already exists — refusing to overwrite.`;
    ...
    if (op === 'branch') {
      ...
      if (!name) return `Missing branch name.\n${usage()}`;
      const parent = ctx.getSession();
      const store = ctx.getSessionStore();
      const provider = ctx.getProvider();
      const summaryModel = ctx.getSummaryModel();
      if (!parent || !store) return 'No active session to branch from.';
      if (!provider) return 'No provider configured — cannot summarise parent context.';
