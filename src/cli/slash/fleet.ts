...
const QUICK_SUBCOMMANDS = new Set(['list', 'help', '--help', '-h', 'discard', 'cleanup', 'merge']);

export const fleetCommand: SlashCommand = {
  name: 'fleet',
  description: 'Parallel agents in git worktrees (`/fleet list`, `/fleet help` here; `dirgha fleet launch` in a shell)',
  async execute(args) {
    ...
    // response (fleetCommand writes to process.stdout directly).
