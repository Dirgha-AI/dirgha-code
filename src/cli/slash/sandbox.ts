...
const MODES: SandboxMode[] = ["off", "auto", "strict"];
...
function describe(mode: SandboxMode): string {
  switch (mode) {
    case "off":
...
export const sandboxCommand: SlashCommand = {
  name: "sandbox",
  description:
    "View or change tool sandbox mode (off | auto | strict). Affects shell/git/lsp; fs-* tools unsandboxed today.",
  async execute(args, ctx) {
    ...
    const next = args[0] as SandboxMode;
