...
export const mcpCommand: SlashCommand = {
  name: "mcp",
  description:
    "Manage MCP connections: /mcp connect | /mcp list | /mcp disconnect | /mcp saved",
  async execute(args, _ctx) {
    ...
    if (sub === "list") {
      if (connections.size === 0) return "No MCP servers connected.";
    ...
    if (sub === "disconnect") {
      ...
      if (name) {
        ...
        if (!conn) return `No MCP server connected with name "${name}".`;
    ...
    if (sub === "saved") {
      ...
      if (entries.length === 0) return "No servers saved in ~/.dirgha/mcp.json.";
