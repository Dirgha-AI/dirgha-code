 ...
 */
function distinctId(sessionId: string): string {
  return createHash('sha256').update(sessionId).digest('hex').slice(0, 16);
}

/** Build the Posthog properties object — strictly the minimal fields. */
function buildProperties(ev: TelemetryEvent): Record<string, string> {
  const out: Record<string, string> = {
    version: ev.version,
    os: ev.os,
    node: ev.node,
  };
  if (ev.command !== undefined) out.command = ev.command;
  if (ev.error_class !== undefined) out.error_class = ev.error_class;
  return out;
}
 ...
 */
export async function sendEvent(ev: TelemetryEvent): Promise<boolean> {
  try {
    const cfg = readTelemetryConfig();
    if (!cfg.enabled) return false;
    if (!cfg.sessionId) return false;
    ...
    if (process.env.DIRGHA_TELEMETRY_DEBUG) {
      ...
      console.error('[telemetry] send failed:', (e as Error).message);
    }
    return false;
  }
}
...
export async function trackCommand(command: string, version: string): Promise<boolean> {
  return sendEvent({
    event: 'cli_command',
    command,
    version,
...
export async function trackError(errorClass: string, command: string, version: string): Promise<boolean> {
  return sendEvent({
    event: 'cli_error',
    command,
    version,
