...
function findGguf(candidates: string[]): string | null {
  for (const c of candidates) {
...
function extractText(content: Message['content']): string {
  ...
  for (const p of content) {
    ...
    else if (p.type === 'thinking') parts.push(p.text);
    else if (p.type === 'tool_result') parts.push(`[Tool result: ${p.content}]`);
  }
  return parts.join('\n');
}
...
function buildChatML(messages: Message[], codexContext: string): string {
  const parts: string[] = [];
  ...
  if (codexContext) systemText = codexContext + '\n\n' + systemText;
...
// ── Qdrant RAG (optional, keyword-based, soft-fail) ──────────────────
...
async function resolveQdrantCollection(): Promise<string | null> {
  try {
...
async function fetchCodexContext(query: string, collection: string): Promise<string> {
  ...
  try {
    ...
    if (!res.ok) return '';
    const body = (await res.json()) as { result?: { points?: QdrantPoint[] } };
    ...
    for (const hit of top) {
      const title =
        ...
        (hit.payload?.['machine_name'] as string) ||
        String(hit.id);
      const text =
        (hit.payload?.['text'] as string) ||
        (hit.payload?.['content'] as string) ||
        (hit.payload?.['description'] as string) ||
        '';
      lines.push(`\n[Codex: ${title}]`);
...
async function* streamLlamaCli(
  ...
  try {
    ...
    throw new ProviderError(
      `Failed to start llama-cli: ${err instanceof Error ? err.message : String(err)}`,
      'machine1',
      0,
      false,
    );
  ...
  const spawnErrRef: { value: Error | null } = { value: null };
  ...
  try {
    for await (const raw of proc.stdout!) {
      if (spawnErrRef.value) break;
      outBuffer += (raw as Buffer).toString('utf8');
  ...
  if (spawnErrRef.value) {
    throw new ProviderError(
      `llama-cli not found or not executable: ${spawnErrRef.value.message}. ` +
      'Install llama.cpp and add llama-cli to your PATH, or set LLAMA_CLI_PATH.',
      'machine1',
      0,
      false,
    );
...
export class Machine1Provider implements Provider {
  ...
  private async *_stream(req: StreamRequest): AsyncGenerator<AgentEvent> {
    ...
    if (!llamaPath) {
      throw new ProviderError(
        ...
        'and ensure llama-cli is on your PATH, or set LLAMA_CLI_PATH=/path/to/llama-cli.',
        'machine1',
        0,
        false,
      );
    ...
    if (!ggufPath) {
      throw new ProviderError(
        'Machine 1 model not found. Place machine1-q4_k_m.gguf in ~/.dirgha/ ' +
        'or set MACHINE1_MODEL_PATH=/absolute/path/to/machine1.gguf.\n' +
        'Download: dirgha pull machine1  (or visit dirgha.ai/models)',
        'machine1',
        0,
        false,
      );
