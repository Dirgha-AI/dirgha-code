...
function splitSystem(messages: Message[]): { system: string | undefined; messages: Message[] } {
  ...
  for (const m of messages) {
    if (m.role === 'system') {
      const t = typeof m.content === 'string'
        ? m.content
        : m.content.filter((p): p is Extract<ContentPart, { type: 'text' }> => p.type === 'text').map(p => p.text).join('');
      systemTexts.push(t);
    } else {
      rest.push(m);
    }
...
function toAnthropicMessage(msg: Message): { role: 'user' | 'assistant'; content: AnthropicBlock[] } {
  ...
  for (const p of msg.content) {
    ...
    else if (p.type === 'tool_use') blocks.push({ type: 'tool_use', id: p.id, name: p.name, input: p.input ?? {} });
    else if (p.type === 'tool_result') blocks.push({ type: 'tool_result', tool_use_id: p.toolUseId, content: p.content });
  }
...
class AnthropicState {
  private blocks = new Map<number, { kind: 'text' | 'tool' | 'thinking'; id?: string; name?: string; buffer: string }>();

  *ingest(ev: AnthropicStreamEvent): Generator<AgentEvent> {
    switch (ev.type) {
      ...
      case 'content_block_stop': {
        ...
        const entry = this.blocks.get(index);
        if (!entry) return;
        if (entry.kind === 'text') yield { type: 'text_end' };
        else if (entry.kind === 'thinking') yield { type: 'thinking_end' };
        else if (entry.kind === 'tool') {
          let input: unknown = {};
          try { input = entry.buffer ? JSON.parse(entry.buffer) : {}; } catch { input = {}; }
