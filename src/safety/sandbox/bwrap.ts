...
export class BwrapSandbox implements SandboxAdapter {
  readonly platform = 'linux-bwrap' as const;

  async available(): Promise<boolean> {
    try { accessSync(BWRAP, constants.X_OK); return true; } catch { return false; }
  }
  ...
  async exec(opts: SandboxExecOptions): Promise<SandboxResult> {
    ...
    if (!opts.networkAllowed) args.push('--unshare-net');
