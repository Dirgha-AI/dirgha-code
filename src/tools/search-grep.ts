...
export const searchGrepTool: Tool = {
  ...
  async execute(rawInput: unknown, ctx): Promise<ToolResult<{ matches: number; truncated: boolean; engine: 'ripgrep' | 'node' }>> {
...
async function runRipgrep(input: Input, root: string, limit: number): Promise<ToolResult<{ matches: number; truncated: boolean; engine: 'ripgrep' | 'node' }> | undefined> {
  ...
  child.stdout.on('data', (buf: Buffer) => { out.push(buf.toString('utf8')); });
  child.stderr.on('data', (buf: Buffer) => { errChunks.push(buf); });
...
async function nodeScan(input: Input, root: string, limit: number): Promise<ToolResult<{ matches: number; truncated: boolean; engine: 'ripgrep' | 'node' }>> {
  const flags = input.ignoreCase ? 'gi' : 'g';
  let regex: RegExp;
  try { regex = new RegExp(input.pattern, flags); } catch (err) {
