...
function smartSlice(abs: string, raw: string): string {
  ...
  if ((name.endsWith('.json') || name.endsWith('.jsonc')) && total > 500) {
    try {
      ...
      const preview = keys.slice(0, 20).map(k => {
        const v = obj[k];
        const type = Array.isArray(v) ? `Array(${v.length})` : typeof v === 'object' && v !== null ? `{${Object.keys(v).length} keys}` : JSON.stringify(v).slice(0, 60);
        return `  "${k}": ${type}`;
      });
    ...
    } catch { /* not valid JSON, fall through to line slicing */ }
  }
  ...
  const head = lines.slice(0, HEAD_LINES).map((l, i) => `${i + 1}\t${l}`).join('\n');
  const tail = lines.slice(-TAIL_LINES).map((l, i) => `${total - TAIL_LINES + i + 1}\t${l}`).join('\n');
...
function guardWrite(abs: string): string | null {
  ...
  if (match) return `Path is read-only (matches: ${match}). Use a different path.`;
  return null;
}

export async function readFileTool(input: Record<string, any>): Promise<ToolResult> {
  try {
    ...
    if (cached !== undefined) {
      return { tool: 'read_file', result: cached.length > 50000 ? cached.slice(0, 50000) + '\n[truncated]' : cached };
    }
  ...
  } catch (e) { return { tool: 'read_file', result: '', error: (e as Error).message }; }
}
...
export function makeDirTool(input: Record<string, any>): ToolResult {
  ...
  } catch (e) { return { tool: 'make_dir', result: '', error: (e as Error).message }; }
}

export function deleteFileTool(input: Record<string, any>): ToolResult {
  try {
    const abs = resolve(input['path'] as string);
    if (statSync(abs).isDirectory()) return { tool: 'delete_file', result: '', error: 'Path is a directory' };
    unlinkSync(abs);
    return { tool: 'delete_file', result: `Deleted: ${abs}` };
  } catch (e) { return { tool: 'delete_file', result: '', error: (e as Error).message }; }
}
