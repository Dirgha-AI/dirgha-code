...
export function distillToolResult(
  ...
  if (process.env.DIRGHA_TOOL_DISTILL_DISABLE === '1') return result;
  ...
  (result as { content: string }).content = newContent;
  if (result.metadata === undefined) {
    (result as { metadata?: Record<string, unknown> }).metadata = {};
  }
  (result.metadata as Record<string, unknown>).distilled = true;
  (result.metadata as Record<string, unknown>).originalLength = content.length;
  if (savedPath) (result.metadata as Record<string, unknown>).fullOutputPath = savedPath;
  return result;
}
