...
function buildArgs(input: Input): string[] {
  ...
  switch (input.action) {
    ...
    case "pr_view": {
      ...
      if (input.state) {
        ...
        if (/^\d+$/.test(input.state)) args.push(input.state);
      }
      return args;
    }
    ...
    case "issue_create": {
      ...
      if (input.label && input.label.length > 0) {
        for (const l of input.label) args.push("--label", l);
      }
      return args;
    }
...
export const githubTool: Tool = {
  name: "github",
  description:
    "Run GitHub operations via gh CLI: create PRs, list issues, view repo info. " +
  ...
  inputSchema: {
    ...
    properties: {
      ...
      base: {
        type: "string",
        description:
          "Target branch for pr_create (defaults to the repo default branch).",
      },
      draft: {
        type: "boolean",
      ...
      state: {
        ...
        description:
          "Filter state for pr_list / issue_list, or PR number for pr_view.",
      },
      limit: {
        type: "integer",
        minimum: 1,
  ...
  async execute(
    rawInput: unknown,
    ctx,
  ): Promise<ToolResult<{ action: Action; exitCode: number; data: unknown }>> {
