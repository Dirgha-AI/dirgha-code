...
export function createExtensionAPI() {
    ...
    const api = {
        ...
        registerSlash(spec) {
            ...
            if (registry.slashes.has(spec.name))
                throw new Error(`duplicate slash name: ${spec.name}`);
            registry.slashes.set(spec.name, spec);
        },
        registerSubcommand(spec) {
            validateName(spec.name, 'subcommand');
            if (registry.subcommands.has(spec.name))
                throw new Error(`duplicate subcommand name: ${spec.name}`);
        ...
        on(event, handler) {
            ...
            if (!bag) {
                bag = new Set();
                registry.listeners.set(event, bag);
            }
            bag.add(handler);
        },
    };
 ...
 */
export function emitEvent(registry, event, payload) {
