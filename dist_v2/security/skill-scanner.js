 ...
 */
export function scanSkillBody(body, meta) {
    const findings = [];
    let score = 0;
    function add(severity, rule, snippet, line) {
        const f = { severity, rule };
        if (snippet !== undefined)
            f.snippet = snippet.slice(0, 80);
    ...
    const checkPipeToShell = (block, startLine) => {
    ...
    if (meta !== undefined) {
        if (!meta.name)
            add('low', 'missing_name', 'meta.name is missing or empty');
        if (!meta.description)
            add('low', 'missing_description', 'meta.description is missing or empty');
        const kw = meta.triggers?.keywords ?? [];
        if (kw.includes('*') || kw.includes(''))
            add('low', 'wildcard_keyword', 'meta.triggers.keywords contains * or empty string');
    }
    ...
    if (critical > 0)
        verdict = 'block';
    else if (high >= 2 || score >= 50)
        verdict = 'block';
    else if (high === 1 || findings.some(f => f.severity === 'medium'))
        verdict = 'warn';
    else
        verdict = 'allow';
