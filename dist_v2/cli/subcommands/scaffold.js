...
function parseArgs(argv) {
    const out = { prompt: '', serve: false, noInstall: false, noAi: false, json: false, help: false };
    const positional = [];
...
function pickTemplate(prompt, override) {
    if (override)
        return TEMPLATES.find(t => t.id === override);
...
function deriveName(prompt, override) {
    if (override)
        return override;
    const slug = prompt
        .toLowerCase()
...
/** Resolve the templates dir whether running from src_v2/ or dist_v2/. */
function templatesDir() {
    const here = dirname(fileURLToPath(import.meta.url));
    // dist_v2/cli/subcommands/scaffold.js → ../../../scaffold-templates (package root)
    // src_v2/cli/subcommands/scaffold.ts → ../../../scaffold-templates (repo root)
    return resolve(here, '..', '..', '..', 'scaffold-templates');
}
async function patchPackageJson(target, name) {
...
export const scaffoldSubcommand = {
    name: 'scaffold',
    ...
    async run(argv) {
        ...
        if (args.help) {
            ...
            for (const t of TEMPLATES)
                stdout.write(`  ${t.id.padEnd(14)} ${t.description}\n`);
            stdout.write(`\nOptions:\n`);
        ...
        if (!args.json)
            stdout.write(`  ${style(defaultTheme.success, '✓')} files copied\n`);
        // Install
        if (!args.noInstall) {
            const isWin = process.platform === 'win32';
            const npmBin = isWin ? 'npm.cmd' : 'npm';
            if (!args.json)
                stdout.write(`  ${style(defaultTheme.muted, 'running:')} npm ${tpl.installCommand.join(' ')}\n`);
            ...
            if (!args.json)
                stdout.write(`  ${style(defaultTheme.success, '✓')} dependencies installed\n`);
        }
        ...
        if (args.serve && !args.noInstall) {
            if (!args.json)
                stdout.write(`  ${style(defaultTheme.muted, 'starting dev server in foreground (Ctrl+C to stop)…')}\n\n`);
            const isWin = process.platform === 'win32';
