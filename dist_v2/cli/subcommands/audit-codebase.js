...
async function listImmediateModules(root) {
    const entries = await readdir(root, { withFileTypes: true }).catch(() => []);
    const out = [];
    for (const e of entries) {
        if (!e.isDirectory())
            continue;
        if (e.name.startsWith('.') || e.name === '__tests__' || e.name === 'node_modules')
            continue;
...
export const auditCodebaseSubcommand = {
    name: 'audit-codebase',
    description: 'Run a parallel-fleet audit across every immediate src module',
    async run(argv) {
        const root = parseArg(argv, 'root') ?? join(process.cwd(), 'src_v2');
        ...
        for (const p of partials) {
            ...
            final.push('');
            if (p.ok)
                final.push(p.markdown.trim());
            else
                final.push(`_(audit failed in ${(p.durationMs / 1000).toFixed(1)}s — partial output captured at \`${join(outDir, p.module + '.md')}\`)_`);
            final.push('');
        }
...
//# sourceMappingURL=audit-codebase.js.map