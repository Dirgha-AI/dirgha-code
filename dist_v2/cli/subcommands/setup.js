...
function parseInteractiveFlag(argv) {
    for (const a of argv) {
        if (a === '--interactive=false' || a === '--non-interactive')
            return false;
        if (a === '--interactive=true' || a === '--interactive')
            return true;
    }
