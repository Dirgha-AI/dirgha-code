...
export function aggregateCost(entries) {
    ...
    for (const entry of entries) {
        ...
        totalTurns++;
        if (entry.ts) {
            if (!windowFromTs || entry.ts < windowFromTs)
                windowFromTs = entry.ts;
            if (!windowToTs || entry.ts > windowToTs)
                windowToTs = entry.ts;
        }
        ...
        // for the price lookup. Same fold-formula as src_v2/cli/subcommands/cost.ts.
...
export function renderCostPage(summary) {
    const esc = (s) => {
        const str = String(s);
