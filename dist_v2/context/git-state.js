...
export function probeGitState(cwd) {
    ...
    if (inside !== 'true')
        return { inRepo: false };
    const branch = run(['rev-parse', '--abbrev-ref', 'HEAD'], cwd).trim() || 'HEAD';
    const dirtyLines = run(['status', '--short'], cwd).split('\n').filter(Boolean).slice(0, SHORT_STATUS_CAP_LINES);
    const recent = run(['log', `-n`, String(RECENT_COMMITS), '--format=%h %s'], cwd).split('\n').filter(Boolean);
 ...
 */
export function renderGitState(state) {
    if (!state.inRepo)
        return '';
    const lines = ['<git_state>'];
    lines.push(`branch: ${state.branch ?? 'unknown'}`);
    if (state.dirty && state.dirty.length > 0) {
        lines.push(`dirty (${state.dirty.length} file${state.dirty.length === 1 ? '' : 's'}):`);
        for (const l of state.dirty)
            lines.push(`  ${l}`);
    }
    else {
