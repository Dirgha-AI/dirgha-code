...
import type { EvalReport, EvalResult, EvalTask } from './types.js';
...
export async function runRegressionSuite(tasks: EvalTask[], opts: RegressionOptions): Promise<EvalReport> {
...
async function runOne(task: EvalTask, providers: ProviderRegistry, model: string): Promise<EvalResult> {
  ...
  const messages = [];
  if (task.system) messages.push({ role: 'system' as const, content: task.system });
  messages.push({ role: 'user' as const, content: task.prompt });
...
async function checkArtifact(cwd: string, task: EvalTask): Promise<{ ok: boolean; reason: string }> {
  const expected = task.expectedArtifact;
  if (!expected) return { ok: true, reason: 'no artifact check required' };
  const content = await readFile(join(cwd, expected.path), 'utf8').catch(() => undefined);
  if (content === undefined) return { ok: false, reason: `artifact missing: ${expected.path}` };
  ...
  for (const ex of excludes) {
    if (content.includes(ex)) return { ok: false, reason: `artifact contains forbidden token: ${ex}` };
  }
  return { ok: true, reason: 'artifact passed' };
}

export function loadTasksFromJson(path: string): EvalTask[] {
  const text = readFileSync(path, 'utf8');
  return JSON.parse(text) as EvalTask[];
}
