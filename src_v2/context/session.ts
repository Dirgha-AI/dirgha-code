...
export type SessionEntry =
  ...
  | { type: 'system'; ts: string; event: string; data?: Record<string, unknown> };
...
export class SessionStore {
  constructor(private readonly dir: string = join(homedir(), '.dirgha', 'sessions')) {}
...
class SessionImpl implements Session {
  ...
  async *replay(): AsyncIterable<SessionEntry> {
    ...
    for (const line of content.split('\n')) {
      ...
      try { yield JSON.parse(line) as SessionEntry; } catch { continue; }
    }
  }
...
export async function streamJsonl(path: string, onLine: (entry: SessionEntry) => void): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    stream.on('data', (chunk: string | Buffer) => {
      ...
      while ((idx = buffer.indexOf('\n')) >= 0) {
        ...
        if (!line.trim()) continue;
        try { onLine(JSON.parse(line) as SessionEntry); } catch { /* skip */ }
      }
    });
    stream.on('end', () => {
      if (buffer.trim()) {
        try { onLine(JSON.parse(buffer) as SessionEntry); } catch { /* skip */ }
      }
      resolve();
    });
