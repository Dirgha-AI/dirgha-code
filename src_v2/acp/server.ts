...
export class AcpServer {
  private sessionsByAcpId = new Map<string, { sessionId: string; history: Message[] }>();
  private usage: UsageTotal = { inputTokens: 0, outputTokens: 0, cachedTokens: 0, costUsd: 0 };
  ...
  start(): void {
    ...
    stdin.on('data', chunk => {
      ...
      while ((idx = buffer.indexOf('\n')) >= 0) {
        ...
        try { void this.handle(JSON.parse(line) as AcpRequest); } catch { /* skip */ }
      }
    });
  }

  private async handle(req: AcpRequest): Promise<void> {
    try {
      switch (req.method) {
        case 'initialize':
          this.reply(req.id, {
  ...
  private async handlePrompt(req: AcpRequest): Promise<void> {
    ...
    if (!active) { this.replyError(req.id, -32602, `Unknown session: ${acpSessionId}`); return; }
  ...
  private reply(id: AcpRequest['id'], result: unknown): void {
    const out: AcpResponse = { jsonrpc: '2.0', id, result };
    stdout.write(`${JSON.stringify(out)}\n`);
  }

  private replyError(id: AcpRequest['id'], code: number, message: string): void {
    const out: AcpResponse = { jsonrpc: '2.0', id, error: { code, message } };
    stdout.write(`${JSON.stringify(out)}\n`);
  }
