...
export type FleetEvent =
  | { type: 'fleet_start'; goal: string; agents: FleetAgent[] }
  | { type: 'fleet_agent_start'; agentId: string; subtask: FleetSubtask; worktreePath: string; branch: string }
  | { type: 'fleet_agent_progress'; agentId: string; status: FleetAgentStatus; bytes: number }
  | { type: 'fleet_agent_end'; agentId: string; status: FleetAgentStatus; error?: string; usage: UsageTotal }
  | { type: 'fleet_end'; goal: string; successCount: number; failCount: number; durationMs: number };
