...
async function readEntries(path: string): Promise<SessionEntry[]> {
  ...
  for (const line of text.split('\n')) {
    ...
    try { out.push(JSON.parse(line) as SessionEntry); } catch { /* skip */ }
  }
  return out;
}
 ...
 */
function truncateAtUserTurn(entries: SessionEntry[], n: number): SessionEntry[] {
  let userTurnsSeen = 0;
  for (let i = entries.length - 1; i >= 0; i--) {
    const e = entries[i];
    if (e.type === 'message' && e.message?.role === 'user') {
      userTurnsSeen++;
      if (userTurnsSeen === n) return entries.slice(0, i);
    }
  }
...
export const undoSubcommand: Subcommand = {
  name: 'undo',
  ...
  async run(argv): Promise<number> {
    ...
    if (!Number.isFinite(n) || n < 1) { stderr.write(`${usage()}\n`); return 1; }

    const sessionId = sessionFlag ?? await mostRecentSession();
    if (!sessionId) { stderr.write('No sessions found in ~/.dirgha/sessions/\n'); return 1; }
    const path = join(sessionsDir(), `${sessionId}.jsonl`);

    const entries = await readEntries(path);
    if (entries.length === 0) { stderr.write(`Session ${sessionId} is empty.\n`); return 1; }

    if (list) {
      const tail = entries.filter(e => e.type === 'message').slice(-10);
      if (json) { stdout.write(`${JSON.stringify({ sessionId, tail })}\n`); return 0; }
    ...
    if (dropped === 0) { stdout.write(style(defaultTheme.muted, `(nothing to undo — fewer than ${n} user turns)\n`)); return 0; }
    ...
    if (json) { stdout.write(`${JSON.stringify({ sessionId, dropped, kept: kept.length, backup })}\n`); return 0; }
