...
export const memorySubcommand: Subcommand = {
  name: 'memory',
  ...
  async run(argv): Promise<number> {
    ...
    if (op === 'show') {
      ...
      if (!entry) { stderr.write(`Memory "${id}" not found.\n`); return 1; }
    ...
    if (op === 'search') {
      ...
      if (hits.length === 0) { stdout.write(`No memories match "${q}".\n`); return 0; }
      for (const m of hits) stdout.write(`  ${m.id} — ${m.name}\n`);
      return 0;
    }
    ...
    if (op === 'remove' || op === 'rm') {
      ...
      if (!existing) { stderr.write(`Memory "${id}" not found.\n`); return 1; }
    ...
    if (op === 'type') {
      ...
      if (!id || !typeArg) { stderr.write(`Missing args.\n${HELP}\n`); return 2; }
