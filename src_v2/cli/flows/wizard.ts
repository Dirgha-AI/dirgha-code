...
function dirghaHome(): string { return join(homedir(), '.dirgha'); }
function keysPath(): string { return join(dirghaHome(), 'keys.json'); }
function configPath(): string { return join(dirghaHome(), 'config.json'); }
...
async function writeJson(path: string, data: unknown, mode?: number): Promise<void> {
  await mkdir(dirghaHome(), { recursive: true });
  await writeFile(path, `${JSON.stringify(data, null, 2)}\n`, 'utf8');
  if (mode !== undefined) {
    try { await chmod(path, mode); } catch { /* non-POSIX */ }
  }
}
...
async function promptHidden(rl: ReturnType<typeof createInterface>, prompt: string): Promise<string> {
...
async function pickProvider(rl: ReturnType<typeof createInterface>): Promise<ProviderChoice | null> {
...
async function pickModel(provider: ProviderChoice, rl: ReturnType<typeof createInterface>): Promise<string | null> {
  ...
  if (suggested !== undefined && !top.includes(suggested)) top.unshift(suggested);
  top.forEach((m, i) => {
    const num = `${i + 1}`.padStart(2);
    const marker = m === suggested ? style(defaultTheme.success, '  ← recommended') : '';
    stdout.write(`  ${num}. ${m}${marker}\n`);
