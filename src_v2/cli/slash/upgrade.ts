...
async function fetchReferralCode(token: string, apiBase: string): Promise<string | undefined> {
  try {
    const res = await jsonRequest<ReferralPayload>({
      baseUrl: apiBase,
    ...
    if (err instanceof IntegrationError) return undefined;
    return undefined;
  }
}
