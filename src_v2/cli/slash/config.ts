...
export const configCommand: SlashCommand = {
  ...
  async execute(args) {
    ...
    if (op === 'path') return path;

    if (op === 'show') {
      const exists = await stat(path).then(() => true).catch(() => false);
      if (!exists) return `No DIRGHA.md at ${path}. Run /init to create one.`;
    ...
    if (op === 'append') {
      ...
      if (!body) return `Missing text to append.\n${usage()}`;
      const exists = await stat(path).then(() => true).catch(() => false);
      if (!exists) return `No DIRGHA.md at ${path}. Run /init first.`;
