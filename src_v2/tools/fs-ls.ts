...
export const fsLsTool: Tool = {
  ...
  async execute(rawInput: unknown, ctx): Promise<ToolResult<{ entries: number }>> {
    ...
    if (!info || !info.isDirectory()) return { content: `Not a directory: ${input.path ?? '.'}`, isError: true };
    ...
    for (const name of visible) {
      ...
      if (!s) continue;
      if (s.isDirectory()) rows.push(`dir   ${name}/`);
      else if (s.isFile()) rows.push(`file  ${name}\t${s.size}`);
