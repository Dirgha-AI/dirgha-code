...
export async function getAccountStatus(token: string, apiBase?: string): Promise<AccountStatus> {
  return jsonRequest<AccountStatus>({
    baseUrl: resolveApiBase(apiBase),
...
export function createBillingContext(sessionId: string, budgetCapUsd?: number): BillingContext {
...
export async function preRequestCheck(
  ctx: BillingContext,
  estTokens: number,
  opts: { token?: string; apiBase?: string; remote?: boolean } = {},
